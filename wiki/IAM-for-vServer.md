 ...
 **1. Bắt đầu sử dụng IAM** Hướng dẫn này nhằm hướng dẫn người dùng có thể nhanh chóng bắt đầu sử dụng IAM trong các dịch vụ vServer bằng cách sử dụng quyền mặc định  **(được định nghĩa bởi VNG Cloud Managed Policies)**  cho hệ thống vServer gọi là  **vServerFullAccess.** 
...
1. Sau khi đăng nhập, bạn sẽ thấy tổng quan về trang web vServer.
1. Thử truy cập vào các trang Network, Server, Bock store, Load balancer, Container & Billing, bạn sẽ thấy thông báo về quyền bị giới hạn như dưới đây.

![](images/storage/image2023-7-24_17-9-44.png)
...
1. Tìm kiếm **chính sách vServerFullAccess**  bằng cách nhập tên chính xác của nó vào ô tìm kiếm.
1. Tick vào kết quả tìm được và nhấp vào nút  **"Attach"**  ở góc dưới bên phải của hộp thoại.

5. Truy cập lại Cổng thông tin vServer với Tài khoản Người dùng IAMTruy cập lại Cổng thông tin vServer bằng cách làm theo hướng dẫn ở Bước 3, sau đó bạn có thể truy cập vào tất cả các phần trong Cổng thông tin vServer sau khi gán chính sách vServerFullAccess cho tài khoản người dùng IAM.
...
* Tìm hiểu thêm về IAM cho vServer: [[Quản lý định danh và truy cập (IAM) cho vServer|Quản-lý-định-danh-và-truy-cập-(IAM)-cho-vServer]]


* Tìm hiểu thêm về IAM:
    * IAM Identity: [[IAM Identities|IAM-Identities]]
    * Quản lý Truy cập IAM: [[IAM Access Management|IAM-Access-Management]]
    * Các trường hợp sử dụng thông thường của IAM: [[Example of Common Use Cases|Example-of-Common-Use-Cases]]
