...
Khi chọn Deployment Type là **Cluster**, hệ thống hiển thị thêm section **IMAGE** cho phép bạn khôi phục cluster từ backup có sẵn:

* Chọn tab **Backup Image** để xem danh sách các backup từ Backup Center.
...
* **Cloud Network (VPC & Subnet)**: Chọn VPC và Subnet cho cluster. Nếu chưa có, bạn có thể tạo mới theo hướng dẫn [tại đây](../../vserver/compute-hcm03-1a/network/virtual-private-cloud-vpc/).
...
Nếu tài khoản của bạn chưa có Backup Location nào ở trạng thái Available (Product = vDB), dropdown Backup Location sẽ hiển thị trống với viền màu đỏ kèm thông báo: **"Vui lòng tạo mới backup location."** Bạn cần click vào link **"Click here to manage your Backup Locations"** để mở Backup Center và tạo mới location trước khi tiếp tục tạo cluster.
{% endhint %}

Nếu chưa có Backup Policy, bạn có thể click vào link **"Click here to manage your Backup Policies"** để mở Backup Center và tạo mới.
...
| **Replication** | Trạng thái replication và độ trễ (lag) giữa Writer và Readers |
...
2. Hệ thống hiển thị dialog xác nhận: _"This will create a full snapshot of [cluster name]"_.
...
* Nút **Back up now** sẽ bị **disabled** khi đang có một backup job (Auto hoặc Manual) đang chạy. Hover vào nút sẽ hiển thị tooltip: _"A backup job is currently in progress"_.
...
2. Hoặc khi tạo cluster mới, chọn restore point tại section **IMAGE** trong Bước 1.
...
2. Hệ thống hiển thị dialog xác nhận với các thành phần:
   ...
   * **Checkbox xác nhận:** _"I acknowledge that upon instance deletion, manual backups and automated backups will no longer be available"_ — bắt buộc tích để tiếp tục.
...
* Checkbox acknowledge sẽ không hiển thị.
* Hệ thống hiển thị note: _"No backup database found. Final backup is unavailable."_
