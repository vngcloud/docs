...
**Resource Tag** cho phép gắn một tập nhãn dạng key/value lên toàn bộ tài nguyên bao gồm server và volume thuộc một **Node Group**. **Resource Tag** sẽ được tự động tạo trên:
...
Bạn có thể dùng **Resource Tag** để:
...
* **Resource Tag** được duy trì liên tục — nếu ai sửa/xoá **Resource Tag** do **VKS** quản lý trên **vServer** console, hệ thống **VKS** tự khôi phục lại các **Resource Tag** này.
* **Volume gắn sau** (**PVC** tạo qua **CSI**) cũng sẽ được hệ thống **VKS** gắn tags tự động.
* **Resource Tag** sẽ được hệ thống **VKS** tự động cập nhật tối đa 15 phút sau khi **Server**/**Volume** được tạo.

{% hint style="info" %}
**Lưu ý cho account IAM-user:**

IAM user cần được cấp đầy đủ các quyền dưới đây (admin nên cấp một lần trước khi sử dụng):
{% endhint %}

| Quyền                     | Mục đích                                           | Dùng tại bước                |
| ------------------------- | -------------------------------------------------- | ---------------------------- |
| `UpdateNodeGroupMetadata` | Cập nhật metadata (Label/Taint/Tag) cho Node Group | Cập nhật metadata Node Group |

#### 2. Hướng dẫn cấu hình

**Tạo Resource Tag**

**Resource Tag** cho **Node Group** được cấu hình khi tạo **Cluster/Node Group**, tại section Default Node Group Configuration → Node Group Metadata Setting (Optional).
...
**Bước 3**: Thêm, sửa hoặc xóa **Resource Tag**

**Bước 4**: Lưu thay đổi

1. Kiểm tra lại danh sách **Resource Tag**.
2. Nhấn **Save** để áp dụng.

#### 3. Tag hệ thống (VKS Managed)

Ngoài **Resource Tag** của bạn, hệ thống **VKS** luôn tự gắn 3 tag sau lên mọi tài nguyên của node group:
...
Hệ thống chỉ gắn tag lên volume do **VKS** quản lý (root disk, data disk, volume **CSI**). Nếu bạn tự gắn một volume khác vào **Server** (không thuộc quản lý của **VKS**), **Volume** này sẽ không được gắn **Resource Tag** của Node Group.

#### 5. Giới hạn số lượng Resource Tag (Quota)
...
Please reduce the number of Resource Tag, or request a tag quota increase on vServer
```
...
| ----------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `UserTagsUpdated`       | Khi bạn đổi Resource Tag và đồng bộ thành công | Xác nhận tag mới đã được áp dụng — message liệt kê tag hiện tại (hoặc "Removed all Resource Tags" nếu bạn xoá hết) |
