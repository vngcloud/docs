...
Với GreenNode, User có thể **Chia sẻ Snapshot** (Share Snapshot) với các tài khoản GreenNode khác. Những tài khoản GreenNode khác có thể sử dụng Snapshot được chia sẻ để tạo nhanh VM để đáp ứng nhu cầu. Bài viết này sẽ mô tả làm thế nào để Share Snapshot và làm thế nào để ngừng Share Snapshot.
...
* File Snapshot chỉ được **chia sẻ cho những tài khoản của GreenNode**;
* User có thể được chia sẻ một file snapshot cho **tối đa 64 tài khoản** GreenNode;
...
* Cần yêu cầu User được chia sẻ file Snapshot cung cấp mã ID và Email của tài khoản GreenNode.
...
* Bước 1: Đăng nhập vào tài khoản GreenNode / chọn Sản phẩm **vServer**;
...
* Bước 6: Điều hướng đến màn hình **GreenNode Help Desk:**
  ...
  * Sau đó nhấn "**Submit**" để gửi đến hệ thống GreenNode để xử lý.
* Bước 7: Sau khi GreenNode xử lý thành công, khi vào lại danh sách Snapshot thì tại file Snapshot yêu cầu chia sẻ, nếu user thấy tại cột "**Chia sẻ với**" (Share to)  có hiển thị thông tin tài khoản user chia sẻ (Owner) và user được chia sẻ, Bên cạnh đó sẽ có email thông báo cho tài khoản được chia sẻ đã nhận file snapshot thành công.
...
* Bước 1: Đăng nhập vào tài khoản GreenNode / chọn Sản phẩm **vServer**;
* Bước 2: Chọn mục **Snapshots / Snapshot**: User xem được giao diện Snapshot;
* Bước 3: Tại file Snapshot muốn ngừng chia sẻ, ở cột "**Chia sẻ với**" (Share to), User click chọn vào icon Account GreenNode;
