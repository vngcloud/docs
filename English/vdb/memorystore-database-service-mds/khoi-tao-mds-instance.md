...
* **Password:** If the password requirement is enabled, enter your password. GreenNode recommends setting a strong password and storing it in a secure location. To ensure information security, the password must meet all minimum requirements for length, allowed characters, special characters, etc.
...
Note: To enhance security, GreenNode allows you to restrict trusted IP addresses that are allowed to access each MDS Instance through Security Group Rules.
...
* **Backup Time:** The time to perform automatic backups. GreenNode recommends configuring this time during the period of lowest system traffic, usually from 12 AM to 5 AM.
...
Congratulations on successfully creating a DB Instance on the GreenNode system. Please refer to the instructions on connecting to the MDS Instance to start using the database.
