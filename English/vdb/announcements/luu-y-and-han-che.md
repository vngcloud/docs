...
For long-time queries, you should use a connection pool mechanism, as well as configure related parameters such as tcp-keepalive, heartbeat, etc., to avoid connection interruptions that cause query timeout, no reply errors,...

For example with redis-py, you can refer to:
