...
* Learn more at [OpenClaw 1-Click](../../ai-stack/agent-base/one-click-openclaw/openclaw-1-click.md).

**vDB - PostgreSQL Cluster (RDS)**

GreenNode has released the **PostgreSQL Cluster** feature for the vDB Relational service, enabling PostgreSQL deployment with a **1 Writer + N Readers** architecture that ensures High Availability and read scalability for production workloads.

* **1 Writer + N Readers architecture:** Supports 2 to 10 nodes with automatic failover when the Writer encounters an issue.
* **Mandatory backup:** Backup Policy and Backup Location must be configured at cluster creation.
* **Restore from backup:** Allows creating a new cluster from an existing restore point in Backup Center.
* **Cluster-specific extensions:** Supports extensions including pg_cron, timescaledb, pgaudit, pg_partman, pg_repack, postgis_raster.
* Learn more at [PostgreSQL Cluster](../../vdb/relational-database-service-rds/postgresql/postgresql-cluster.md).
...
* Learn more at [Agentbase](../../ai-stack/agent-base/README.md).
...
GreenNode has released **OpenClaw 1-Click** on Agentbase, bringing a one-click AI Agent deployment experience in just 40–60 seconds directly from the Agent Marketplace. Auto-integrated with GreenNode MaaS, supports BYOK, and connects to Telegram and Zalo from the setup step. Learn more at [OpenClaw 1-Click](../../ai-stack/agent-base/one-click-openclaw/openclaw-1-click.md).

#### March 2026

**Agentbase**

GreenNode has released **Agentbase** — a purpose-built infrastructure platform for AI Agents, enabling developers to deploy and operate AI Agents on the cloud without managing servers, scaling, or credentials. Agentbase includes built-in Identity Service, Runtime Service, and Memory Service, with direct integration to GreenNode MaaS (OpenAI-compatible). Learn more at [Agentbase](../../ai-stack/agent-base/README.md).
